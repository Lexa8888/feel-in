...
const MOOD_RECOMMENDATIONS = { happy: ["📸 Фото", "📅 Свидание"], sad: ["💓 Пульс", "💌 Сообщение"], love: ["✨ Ритуал", "📝 Дневник"], hug: ["🫂 Объятия", "🎵 Музыка"], adore: ["💖 Комплимент", "🌹 Воспоминания"], angry: ["🧘 Вдох", "🕊️ Мир"], sleepy: ["🌙 Ночи", "💤 Сон"], celebrate: ["🎊 Радость", "🥂 Праздник"] };
...
const AnimatedConfetti = ({ config }) => {
  const animY = useRef(new Animated.Value(config.y)).current; const animRot = useRef(new Animated.Value(config.rotation)).current; const durationValue = 2000; const rotationValue = 360;
  useEffect(() => { Animated.parallel([Animated.timing(animY, { toValue: 800, duration: durationValue / config.speed * 2, useNativeDriver: true }), Animated.timing(animRot, { toValue: config.rotation + rotationValue, duration: durationValue, useNativeDriver: true })]).start(); }, []);
...
function AppContent() {
  ...
  const connectSocket = useCallback((code) => {
    ...
    console.log('🔌 Connecting:', code, 'role:', userRole);
    try {
      ...
      // ✅ ИСПРАВЛЕНО 2: Настроение теперь правильно распределяется по ячейкам
      newSocket.on('status-updated', (d) => {
        if (!d || !d.user) return;
        // statusA всегда для 'M', statusB всегда для 'Ж'
      ...
      newSocket.on('peace-updated', (d) => {
        ...
        if (d.active && d.from !== userRole) Alert.alert('🕊️ Мир!', `${d.from === 'M' ? 'Он' : 'Она'} хочет помириться!`);
      });

      // ✅ ИСПРАВЛЕНО 4: История ритуалов общая и синхронизируется
      newSocket.on('ritual-updated', (entry) => {
        setRitualHistory(prev => {
          if (prev.some(r => r.id === entry.id)) return prev;
          const updated = [entry, ...prev].slice(0, 10);
          AsyncStorage.setItem('ritualHistory', JSON.stringify(updated));
          return updated;
        });
      ...
      newSocket.on('sleep-updated', (p) => { if(!p) return; setPartnerSleeping(p.active === true && p.user && p.user !== userRole); });
      
      // ✅ ИСПРАВЛЕНО 1: Викторина теперь корректно обновляет состояние
      newSocket.on('quiz-updated', ({ quiz, updatedBy }) => {
        if (!quiz) return;
        const today = new Date().toDateString();
        const questionIdx = new Date().getDate() % DAILY_QUESTIONS.length;
        const myField = `ans_${userRole === 'M' ? 'a' : 'b'}`;
        const partnerField = `ans_${userRole === 'M' ? 'b' : 'a'}`;
        ...
        const newState = {
          ...
          myAns: quiz[myField] || quizState.myAns,
          partnerAns: quiz[partnerField] || quizState.partnerAns
        };
        setQuizState(newState);
        AsyncStorage.setItem('dailyQuiz', JSON.stringify(newState));
      });
  ...
  useEffect(() => { 
    let sc=0; if(statusA&&statusB&&statusA===statusB)sc+=40; else if(statusA||statusB)sc+=15; 
    sc+=Math.min(streak*3,30); sc+=Math.min((messages?.length||0)*0.25,30); sc=Math.min(Math.max(sc,0),100); setPulseScore(sc); 
    if(animationRef.current)animationRef.current.stop(); 
    if(sc>5){ const dur=2000-(sc/100)*800,sc2=1.1+(sc/100)*0.3; const lp=Animated.loop(Animated.sequence([Animated.timing(pulseAnim,{toValue:sc2,duration:dur,useNativeDriver:!IS_WEB}),Animated.timing(pulseAnim,{toValue:1,duration:dur,useNativeDriver:!IS_WEB})]));lp.start();animationRef.current=lp; } else { pulseAnim.setValue(1); }
  ...
  const handleJoinWithGender = useCallback(async (gender) => {
    ...
    setLoading(true);
    try {
      const r = await fetch(`${CONFIG.SERVER_URL}/api/pair/join`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({code: pendingPairData.code}) });
    ...
    } catch(e) { Alert.alert('Ошибка', 'Не удалось войти.'); } finally { setLoading(false); }
  }, [pendingPairData]);
        ...
        {currentScreen==='welcome'&&(
          <Animated.View style={[styles.center,{opacity:fadeIn}]}>
            ...
            <View style={styles.btns}>
              <GradientButton onPress={async()=>{setLoading(true);try{const r=await fetch(`${CONFIG.SERVER_URL}/api/pair/create`,{method:'POST',headers:{'Content-Type':'application/json'}});if(!r.ok)throw new Error();const d=await r.json();setPairCode(d.code);setUserRole('M');setCurrentScreen('main');await secureSet('pairData',JSON.stringify({pairCode:d.code,userRole:'M',timestamp:Date.now()}));if(!await AsyncStorage.getItem('relationshipStart'))await AsyncStorage.setItem('relationshipStart',new Date().toISOString());connectSocket(d.code);}catch(e){Alert.alert('Ошибка','Не удалось.');}finally{setLoading(false);}}} title={loading?'...':t('createPair')} colors={[colors.primary,'#ff6b9d']} disabled={loading}/>
              <TouchableOpacity onPress={()=>setCurrentScreen('join')} style={[styles.btnSecondary,{backgroundColor:colors.bgCard,borderColor:colors.border}]}><Text style={[styles.btnSecondaryText,{color:colors.text}]} numberOfLines={1}>{t('join')}</Text></TouchableOpacity>
            </View>
          </Animated.View>
        )}
        {currentScreen==='join'&&(
          <View style={[styles.container,{backgroundColor:colors.bg}]}>
            ...
            <View style={styles.center}><Text style={[styles.sectionTitle,{color:colors.text, textAlign: 'center'}]} numberOfLines={1}>{t('codeLabel')}</Text><TextInput style={[styles.input,{backgroundColor:colors.bgCard,color:colors.text,borderColor:colors.border, textAlign: 'center'}]} placeholder="FEEL-XXXX" placeholderTextColor={colors.textMuted} value={pairCode} onChangeText={setPairCode} autoCapitalize="characters" maxLength={12}/><GradientButton onPress={async()=>{ let cleanCode = pairCode.trim().toUpperCase(); if (!cleanCode.startsWith('FEEL-')) cleanCode = 'FEEL-' + cleanCode.replace('FEEL-', ''); if(!cleanCode) return Alert.alert('Ошибка','Введите код'); setPendingPairData({code: cleanCode}); setShowGenderModal(true); }} title={loading?'...':t('join')} colors={[colors.secondary,'#48cae4']} disabled={loading}/></View>
          </View>
        )}
        {currentScreen==='main'&&(
          <View style={[styles.container,{backgroundColor:colors.bg}]}>
            ...
            <ScrollView style={styles.scroll} contentContainerStyle={{padding:12}} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{if(socketRef.current){socketRef.current.emit('load-messages',{pairCode});socketRef.current.emit('get-profiles',{pairCode});}setRefreshing(false);}} tintColor={colors.primary}/>}>
              <View style={[styles.pulseContainer,{backgroundColor:colors.bgCard,borderColor:colors.border}]}>
                ...
                <Animated.View style={{transform:[{scale:pulseAnim}]}}><LinearGradient colors={[statusA===statusB&&statusA?colors.primary:colors.textMuted,colors.primary]} style={styles.pulseHeartGradient}><Text style={styles.pulseHeart}>💓</Text></LinearGradient></Animated.View>
              ...
              <Card style={{backgroundColor:colors.bgCard,borderColor:colors.border}}>
                ...
                <View style={styles.statusRow}>
                  <View style={[styles.statusBadge,{backgroundColor:colors.bg,borderLeftColor:myAvatarColor,borderLeftWidth:3,borderColor:colors.border}]}><Text style={[styles.statusText,{color:myAvatarColor}]} numberOfLines={1}>{t('myStatus')}</Text><Text style={styles.statusEmoji}>{statusA||'—'}</Text></View>
                  <View style={styles.statusDivider}><Text style={[styles.dividerDot,{color:colors.textMuted}]}>•</Text></View>
                  <View style={[styles.statusBadge,{backgroundColor:colors.bg,borderLeftColor:partnerAvatarColor,borderLeftWidth:3,borderColor:colors.border}]}><Text style={[styles.statusText,{color:partnerAvatarColor}]} numberOfLines={1}>{t('partnerStatus')}</Text><Text style={styles.statusEmoji}>{statusB||'—'}</Text></View>
                </View>
                ...
                <View style={styles.moodGrid}>{MOOD_EMOJIS.map(item=><TouchableOpacity key={item.key} style={[styles.moodBtn,{backgroundColor:colors.bg,borderColor:colors.border},(statusA===item.emoji||statusB===item.emoji)?{backgroundColor:colors.primary+'40',borderColor:colors.primary}:{}]} onPress={()=>{socketRef.current?.emit('update-status',{code:pairCode,user:userRole,value:item.emoji}); if(userRole==='M')setStatusA(item.emoji);else setStatusB(item.emoji);}}><Text style={styles.moodEmoji}>{item.emoji}</Text></TouchableOpacity>)}</View>
              </Card>
              <Card style={{backgroundColor:colors.bgCard,borderColor:colors.border}}>
                <Text style={[styles.cardTitle,{color:colors.text}]} numberOfLines={1}>{t('chat')} {uploadingMedia&&' ⏳'}</Text>
                {partnerSleeping?(<View style={[styles.sleepBlock,{backgroundColor:colors.bg}]}><Text style={{fontSize:40}}>🌙</Text><Text style={[styles.sleepBlockTitle,{color:colors.accent}]} numberOfLines={1}>{t('sleep')}</Text></View>):(<View style={[styles.chatBox,{backgroundColor:colors.bg,borderColor:colors.border}]}>
                ...
                {!partnerSleeping&&<View style={styles.chatInputRow}>
                  <TouchableOpacity onPress={()=>{Alert.alert('Фото', 'Функция в разработке');}} style={[styles.attachBtn,{backgroundColor:colors.bgCard}]}><Text style={[styles.attachText,{color: colors.textSecondary, fontWeight: '600'}]} numberOfLines={1}>📷</Text></TouchableOpacity>
              ...
              <Card style={{backgroundColor:colors.bgCard,borderColor:colors.border}}>
                <View style={styles.cardHeader}><Text style={[styles.cardTitle,{color:colors.text}]} numberOfLines={1}>{t('dates')}</Text><TouchableOpacity onPress={()=>setShowAddDateModal(true)}><Text style={[styles.addText,{color:colors.secondary}]} numberOfLines={1}>{t('addDate')}</Text></TouchableOpacity></View>
              ...
              <TouchableOpacity onPress={()=>{if(!socketRef.current)return; socketRef.current.emit('sleep-toggle',{pairCode,user:userRole,active:true}); Alert.alert('🌙 Режим сна');}} style={[styles.actionButton,{marginTop:10,borderColor:colors.border}]}><LinearGradient colors={themeMode==='light'?['#e5e7eb','#f3f4f6']:['#1a1a35','#2d2d55']} style={styles.actionButtonGradient}><Text style={{fontSize:28,marginBottom:6}}>🌙</Text><Text style={{color:themeMode==='light'?colors.textSecondary:colors.accent,fontWeight:'700'}}>{t('sleep')}</Text></LinearGradient></TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  card:{borderRadius:18,padding:16,marginBottom:14,borderWidth:1},cardHeader:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:10},cardTitle:{fontSize:18,fontWeight:'700',marginBottom:10},addText:{fontSize:16,fontWeight:'600'},
