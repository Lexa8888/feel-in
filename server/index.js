...
io.on('connection', (socket) => {
  ...
  socket.on('update-status', async ({ code, user, value }) => {
    try {
      ...
      if (error) {
        ...
        console.log('✅ Status updated, broadcasting to room:', code);
        // Broadcast to everyone in the room EXCEPT sender
        socket.to(code).emit('status-updated', { user, value });
      }
    } catch (err) { 
  ...
  socket.on('sleep-toggle', async ({ pairCode, user, active, sleepUntil }) => {
    try {
      ...
      io.to(pairCode).emit('sleep-updated', { 
        active, 
        user, 
        sleepUntil 
      });
